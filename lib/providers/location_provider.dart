...
class LocationProvider extends ChangeNotifier {
  ...
  Future<bool> getCurrentLocation() async {
    try {
      ...
      if (_currentPosition != null) {
        _currentAccuracy = _currentPosition!.accuracy < 5
      ...
      _setError('Failed to get your current location: $e');
      return false;
    } finally {
      _setLoading(false);
    }
  ...
  Future<bool> startLocationTracking({int retries = 3}) async {
    try {
      if (isTracking) {
        return true;
      }
        ...
        try {
          ...
          _positionStreamSubscription = positionStream.listen(
            (Position position) {
              ...
              _currentAccuracy = position.accuracy < 5
                  ? LocationAccuracy.best
          ...
          attempt++;
          if (attempt >= retries) {
            _setError('Failed to start location tracking after $retries attempts.');
            return false;
          }
      ...
      _setError('Could not start location tracking: $e');
      return false;
    }
  }
  ...
  /// Sets a target position for distance calculations.
  /// [latitude] must be between -90 and 90 degrees.
  /// [longitude] must be between -180 and 180 degrees.
  void setTargetPosition(double latitude, double longitude) {
    // Validate latitude and longitude
    if (latitude < -90 || latitude > 90 || longitude < -180 || longitude > 180) {
      _setError('Invalid latitude or longitude values.');
      return;
    }
  ...
  void _calculateDistanceToTarget() {
    ...
    try {
      ...
      _lastCalculatedDistance = null;
      _setError('Failed to calculate distance.');
    }
  }
  ...
  Future<bool> isLocationAccurate({double maxAccuracyMeters = 10.0}) async {
    try {
      if (_currentPosition == null) {
        return false;
      }
      bool isAccurate = await _locationService.isLocationAccurate();
      return isAccurate && _currentPosition!.accuracy <= maxAccuracyMeters;
    } catch (e) {
      _setError('Failed to check location accuracy: $e');
      return false;
    }
  }
  ...
  void dispose() {
    stopLocationTracking();
    _locationService.dispose(); // Assuming LocationService has a dispose method
    super.dispose();
  }
}  bool get hasPermission => _hasPermission;
  bool get isLocationEnabled => _isLocationEnabled;
